...
def render_predicted(cfg,
    ...
    """
    renderer_backend = getattr(cfg.model, "renderer_backend", "cuda")
    if renderer_backend in {"torch", "npu_torch"}:
        from models.decoder.npu_differentiable_renderer import render_predicted_torch
        return render_predicted_torch(
            cfg, pc, world_view_transform, full_proj_transform, proj_mtrx,
            camera_center, fov, img_size, bg_color, max_sh_degree,
            scaling_modifier, override_color,
        )
    if renderer_backend != "cuda":
        raise ValueError(
            f"Unknown renderer_backend={renderer_backend!r}; use 'cuda' or 'npu_torch'."
        )
    if GaussianRasterizer is None:
        raise RuntimeError(
            "diff_gaussian_rasterization is unavailable. The native Flash3D "
            "renderer requires CUDA; use benchmark_cpu.py for CPU rendering."
        )
