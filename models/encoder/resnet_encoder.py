...
class ResnetEncoder(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, bn_order, num_input_images=1):
        ...
        if num_input_images > 1:
            ...
            # Respect the caller's flag.  CPU/offline inference later restores
            # this encoder from Flash3D's own RE10K checkpoint, so an ImageNet
            # download is both unnecessary and harmful to reproducibility.
            self.encoder = model(weights=weights if pretrained else None)
