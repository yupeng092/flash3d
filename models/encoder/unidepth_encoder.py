...
class UniDepthExtended(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        # Keep CPU/offline benchmarks independent of TorchHub and Hugging Face.
        # ``fetch_unidepth_source.py`` retrieves only the model definition; the
        # user-supplied V1 ViT-L/14 checkpoint remains in ``weights``.
        project_root = Path(__file__).resolve().parents[2]
        local_repo = project_root / "third_party" / "unidepth_offline"
        local_weight = (
            project_root
            / "weights"
            / "unidepth-v1-cnvnxtl"
        ...
        if not local_repo.joinpath("hubconf.py").is_file():
            raise FileNotFoundError(
                f"Missing offline UniDepth source at {local_repo}. "
                "Run third_party/fetch_unidepth_source.py first."
            )
        if not local_weight.is_file():
            raise FileNotFoundError(
                f"Missing UniDepth V1 ViT-L/14 checkpoint: {local_weight}"
            )
        ...
        self.unidepth.load_pretrained(str(local_weight))
